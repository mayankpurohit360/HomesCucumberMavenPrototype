@completed @regression @portal @forrent @frc
Feature: Submitting Check Availability Lead from SRP on FRC listing
  ...
  Scenario: Submitting An FRC Request Info Property Lead
    When I click the Request Info Button on FRC SRP
    And I see the Agent Contact Form
    And I enter "Test Lead" in the "name" field on the SRP
    And I enter "test@regr.com" in the "froverlay_email" field on the SRP
    And I enter "1234567890" in the "phone" field on the SRP
    And I click the send button from the agent card on SPR for FRC
  ...
  Scenario Outline: Submitting An FRC Request Info Property Lead unchecking
    When I click the Request Info Button on FRC SRP
    And I see the Agent Contact Form
